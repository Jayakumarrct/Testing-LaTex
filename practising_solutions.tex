...
%==============================================================================
% GEOMETRY CHEAT SHEET - TRIANGLE SIMILARITY AND RELATED THEOREMS
%==============================================================================
...
\subsection*{7. Common Diagram Patterns}

\subsubsection*{Right Triangle with Altitude}
\begin{center}
...
\end{tikzpicture}
\end{center}
$\implies CD^2 = AD \times DB$

\subsubsection*{Triangle with Median}
\begin{center}
\begin{tikzpicture}[scale=0.8]
\coordinate (A) at (0,2); \coordinate (B) at (-1,0); \coordinate (C) at (2,0);
...
\end{tikzpicture}
\end{center}
$\implies AM = \dfrac{2}{3} \times \text{median length}$

\subsubsection*{Trapezium with Diagonals}
\begin{center}
\begin{tikzpicture}[scale=0.8]
\coordinate (A) at (0,2); \coordinate (B) at (3,2); \coordinate (C) at (2,0); \coordinate (D) at (1,0);
\path[name path=AC] (A)--(C); \path[name path=BD] (B)--(D);
...
\end{tikzpicture}
\end{center}
$\implies \dfrac{AO}{OC} = \dfrac{BO}{OD}$

\subsection*{8. Essential Problem-Solving Tips}

\begin{itemize}
\item \textbf{Draw diagrams:} Always sketch the figure with given information
\item \textbf{Mark equal measures:} Use same symbols for equal angles/sides
\item \textbf{Look for right angles:} Check for Pythagoras applications
\item \textbf{Identify parallel lines:} Use alternate interior angles
\item \textbf{Set up proportions:} Match corresponding parts
\item \textbf{Cross-multiply carefully:} Ensure correct correspondence
\item \textbf{Verify your answer:} Check if it satisfies all conditions
\end{itemize}
